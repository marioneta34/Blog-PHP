...
<!-- // Pequeño banner decorativo-->
    <div class="text-center mb-3"  style = "background-image: url('images/deco.png'); width:100%; height: 400px ">
        <h1  style="padding-top: 150px;"> Mantente informado!!</h1>
    </div>
...
<!-- // Seccion de Post ordenados por fecha en orden mas reciente-->    
    ...
    <?php   
        ...
        $query ="SELECT * FROM publicaciones where user_id='$usuario' order by creado desc"; 
    ...
    <?php
        ...
        $query ="SELECT firstname, lastname FROM users where id = '$id_usuario'"; 
    ...
    ?>        
        <div class="row border  m-3 shadow-sm p-3 bg-white rounded" style="background: white;">
            ...
            <div class="container float-right w-100  mr-2"> 
                <i class="fa fa-user-o"></i> <?php while ($row = mysqli_fetch_array($result2)){ ?><?php echo $row['firstname']?> <?php echo $row['lastname']?>
                                            
                                            <?php } ?>  
                    &nbsp;
                <i class="fa fa-calendar"></i> <?php echo $fechacreada ?>  Actualizado:  <?php echo $actualizado ?> Categoria: <?php while ($rows= mysqli_fetch_array($categoria)){ ?> <a href="categorias.php?id=<?php echo $id_cate?>"><?php echo $rows['nombre'] ?></a>
                            <?php } ?>
